		...
		for(Node outcome : outcomes) {
			try (Transaction tx = g.beginTx()) {
				...
				for(Relationship rel : outcome.getRelationships(Direction.INCOMING,relTypes.toArray(new RelationshipType[relTypes.size()])) ){
					...
					double effect = (double)rel.getProperty("weight",0.0);
					...
					for(Relationship r : neighbour.getRelationships(Direction.INCOMING, relTypes.toArray(new RelationshipType[relTypes.size()]))){
						double mirEffect = effect * (double)r.getProperty("weight", 0.0);
